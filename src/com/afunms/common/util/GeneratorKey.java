...
public class GeneratorKey
{
   ...
   public synchronized int getTimingKey()
   {
	   ...
	   try
	   {
		   ResultSet rs = db.executeQuery("select id from sys_timingbackup_telnetconfig order by id desc limit 1");
		   if(rs.next())
		   {
			   id = rs.getInt(1);
   ...
   public synchronized int getNextKey(String table)
   {
	   ...
	   try
	   {
		   ResultSet rs = db.executeQuery("select id from " + table + " order by id desc limit 1");
		   if(rs.next())
		   {
			   id = rs.getInt(1);
		   }
